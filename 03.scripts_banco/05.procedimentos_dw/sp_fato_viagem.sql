...
BEGIN
	...
	INSERT INTO FATO_VIAGEM(ID_LOCAL_ORIGEM, ID_LOCAL_DESTINO, ID_STATUS, ID_MOTORISTA, ID_DATA_PARTIDA, ID_DATA_CHEGADA, 
							ID_DATA_DISPONIVEL, QUANTIDADE_DE_VAGAS, VALOR)
	...
	ON AV.LOCAL_ORIGEM = DLO.LOCAL 
	INNER JOIN DIM_LOCAL DLD
	ON AV.LOCAL_DESTINO = DLD.LOCAL
	INNER JOIN DIM_STATUS DS
	ON AV.COD_STATUS = DS.COD_STATUS_VIAGEM
    INNER JOIN DIM_MOTORISTA DM
    ON AV.COD_MOTORISTA = DM.COD_MOTORISTA
    INNER JOIN DIM_TEMPO DTP
    ON AV.DATA_PARTIDA = DTP.DATA
    INNER JOIN DIM_TEMPO DTC
    ON AV.DATA_CHEGADA = DTC.DATA
    INNER JOIN DIM_TEMPO DTD
    ON AV.DATA_DISPONIVEL = DTD.DATA
    WHERE AV.DATA_CARGA = @DATA_CARGA
    GROUP BY DLO.ID_LOCAL, DLD.ID_LOCAL, DS.ID_STATUS, DM.ID_MOTORISTA, DTP.ID_TEMPO, 
			DTC.ID_TEMPO, DTD.ID_TEMPO, AV.TOTAL_VAGAS, AV.VALOR_PASSAGEM
END

EXEC SP_FATO_VIAGEM '20240322'
