...
INNER JOIN DIM_TEMPO DT
ON FV.ID_DATA_PARTIDA = DT.ID_TEMPO
WHERE DS.STATUS = 'CONCLUIDA' AND YEAR(DT.DATA) = YEAR(GETDATE())-1 AND FV.VALOR > 30.00
...
INNER JOIN DIM_TEMPO DT
ON FV.ID_DATA_PARTIDA = DT.ID_TEMPO
WHERE DS.STATUS = 'CONCLUIDA' AND DT.DATA >= DATEADD(MONTH, -6, CONVERT(DATE, GETDATE())) AND FV.VALOR < 30.00
